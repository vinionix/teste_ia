...
async def fake_run_benchmark(
    ...
    return BenchmarkResponse(
        rows=[],
        details={model: [] for model in models},
        environment={"top_k": top_k, "received_models": models},
    )


def make_client(monkeypatch) -> TestClient:
    monkeypatch.setattr(app_module, "client", FakeOllamaClient())
    monkeypatch.setattr(app_module, "run_benchmark", fake_run_benchmark)
...
def test_benchmark_requires_at_least_one_model(monkeypatch):
    ...
    assert response.json()["detail"] == "Selecione pelo menos um modelo instalado."


def test_benchmark_rejects_models_not_installed(monkeypatch):
    client = make_client(monkeypatch)
    response = client.post("/api/benchmark?models=nao-instalado%3A1b&top_k=3")
...
def test_benchmark_validates_top_k_lower_bound(monkeypatch):
    client = make_client(monkeypatch)
    response = client.post("/api/benchmark?models=qwen3%3A0.6b&top_k=0")
...
def test_benchmark_validates_top_k_upper_bound(monkeypatch):
    client = make_client(monkeypatch)
    response = client.post("/api/benchmark?models=qwen3%3A0.6b&top_k=11")
